...
export default function ContactModal({ contact, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    if (contact) {
      ...
      (Object.keys(EMPTY) as (keyof typeof EMPTY)[]).forEach(k => {
        (f as Record<string, unknown>)[k] = (contact as Record<string, unknown>)[k] ?? '';
      });
      setForm(f);
    } else {
      setForm(EMPTY);
  ...
  const Sel = ({ label, field, options }: {
    ...
    <div>
      <label>{label}</label>
      <select className="input" value={(form[field] as string) || ''} onChange={e => set(field, e.target.value)}>
          ...
          {tab === 'identity' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div>
                ...
                <textarea className="input" placeholder="Any background about this contact…" value={form.notes || ''} onChange={e => set('notes', e.target.value)} />
              </div>
            </div>
          )}
